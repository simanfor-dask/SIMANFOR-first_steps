...
# para identificar cada uno de los escenarios, vamos a extraer el nº de escenario y crear una columna con su valor
final_plots$n_scnr <- substr(final_plots$Nombre_fichero_origen, 38, 39)

# calculamos ahora el promedio, por escenario, de volumen y área basimétrica acumulada
mean_acum_by_scnr <- ddply(final_plots, c('n_scnr'), summarise,  # agrupamos por código de escenario
...
ggplot(mean_acum_by_scnr,  # seleccionamos los datos a utilizar 
       ...
       # selección de variables
       aes(G_acum,  # eje X e Y
           group = n_scnr, colour = n_scnr)) +  # agrupamos por tipo de escenario, asignando un color distinto a cada uno
  
  # ponemos las etiquetas   
  labs(title = "Área basimétrica acumulada durante el escenario selvícola",  # título
       # subtitle = "",  # subtítulo
       x = "Área basimétrica (m2/ha)",  # eje x 
       y = ""  # eje y       
  ) +
...
ggplot(mean_acum_by_scnr,  # seleccionamos los datos a utilizar 
       ...
       # selección de variables
       aes(V_acum,  # eje X e Y
           group = n_scnr, colour = n_scnr)) +  # agrupamos por tipo de escenario, asignando un color distinto a cada uno
  
  # ponemos las etiquetas   
  labs(title = "Volumen acumulado durante el escenario selvícola",  # título
       # subtitle = "",  # subtítulo
       x = "Volumen (m3/ha)",  # eje x 
       y = ""  # eje y       
  ) +
